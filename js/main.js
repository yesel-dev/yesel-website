...
const chatbotFAQ = {
    // Programs
    programs: {
        keywords: ['program', 'course', 'class', 'offer', 'teach', 'learn', 'what do you', 'subjects'],
...
We also offer Holiday Camps and School Partnership programs. Would you like details on a specific program?`
    ...
    // Enrollment
    enrollment: {
        keywords: ['enroll', 'enrol', 'register', 'sign up', 'join', 'how to start', 'admission', 'apply'],
...
Would you like to start the enrollment process?`
    ...
    // Schedule
    schedule: {
        keywords: ['schedule', 'time', 'when', 'hours', 'days', 'weekend', 'duration', 'how long'],
...
Classes are typically held once or twice a week. Contact us for the current term schedule!`
    ...
    // Pricing
    pricing: {
        keywords: ['price', 'cost', 'fee', 'payment', 'how much', 'afford', 'charge', 'pay'],
...
We'll find an option that works for you!`
    ...
    requirements: {
        keywords: ['require', 'need', 'material', 'equipment', 'bring', 'prerequisite', 'computer', 'laptop'],
...
Any specific questions about requirements?`
    ...
    // Location
    location: {
        keywords: ['where', 'location', 'place', 'venue', 'address', 'directions', 'come', 'visit'],
...
<strong>Online Option:</strong> Some coding classes are available online.`
    ...
    // Age groups
    age: {
        keywords: ['age', 'old', 'year', 'form 1', 'form 2', 'form 3', 'form 4', 'secondary', 'college', 'university', 'young', 'child'],
...
Which age group are you interested in?`
    ...
    // Holiday camps
    holiday: {
        keywords: ['holiday', 'camp', 'vacation', 'april', 'august', 'december', 'break'],
...
Contact us to reserve a spot for the next holiday!`
    ...
    // Schools / Partnership
    school: {
        keywords: ['school', 'partner', 'club', 'workshop', 'teacher', 'institution'],
...
📞 +254 700 000 000`
    ...
    // Greeting
    greeting: {
        keywords: ['hello', 'hi', 'hey', 'jambo', 'habari', 'good morning', 'good afternoon', 'good evening'],
...
We'd love to have you join the Yesel family!`
    ...
    // Contact
    contact: {
        keywords: ['contact', 'reach', 'call', 'email', 'whatsapp', 'phone', 'talk to someone', 'human', 'person'],
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.card, .teach-card, .approach-card').forEach(el => {
        el.style.opacity = '0';
        observer.observe(el);
    });
});
